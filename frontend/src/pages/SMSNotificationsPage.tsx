    ...
    <div className="space-y-6">
      <div className="page-header">
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs text-emerald-400 bg-emerald-900/30 px-3 py-1.5 rounded-full border border-emerald-800/50 flex items-center gap-1.5">
      ...
      {tab === "templates" && (
        <div className="space-y-3">
          <p className="text-sm text-slate-400">Toggle automated SMS messages. Active templates are sent automatically when the trigger event occurs.</p>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-1">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full border ${t.active ? "bg-emerald-900/30 text-emerald-400 border-emerald-800/40" : "bg-slate-800 text-slate-500 border-slate-700"}`}>
                      {t.active ? "Active" : "Disabled"}
                    </span>
                  </div>
                  <p className="text-xs text-indigo-400 mb-2"><Zap size={10} className="inline mr-1" />{t.trigger}</p>
                  <div className="bg-slate-800/50 rounded-lg p-3 font-mono text-xs text-slate-300 leading-relaxed">{t.message}</div>
                </div>
      ...
      {tab === "send" && (
        <div className="max-w-lg">
          {sent && (
            <div className="mb-4 bg-emerald-900/20 border border-emerald-700/40 rounded-xl p-3.5 flex items-center gap-2.5 text-emerald-300 text-sm">
              <CheckCircle size={15} /> SMS sent successfully!
            </div>
          )}
          <form onSubmit={handleSend} className="philix-card p-5 space-y-4">
            <h3 className="font-semibold text-slate-200 flex items-center gap-2"><Phone size={16} className="text-indigo-400" /> Send Manual SMS</h3>
            <div>
              <label className="text-sm font-medium text-slate-400 mb-1.5 block">Recipient Phone Number</label>
            ...
            <div>
              <label className="text-sm font-medium text-slate-400 mb-1.5 block">Message <span className="text-slate-600">({manualMessage.length}/160)</span></label>
            ...
            <p className="text-xs text-slate-600">Estimated cost: K0.10 per SMS · Sender ID: PhilixFin</p>
          </form>

          <div className="mt-4 philix-card p-4">
            <h4 className="text-sm font-semibold text-slate-300 mb-3 flex items-center gap-2"><Users size={14} className="text-indigo-400" /> Bulk SMS</h4>
            <p className="text-xs text-slate-500 mb-3">Send to groups of clients by status or loan state.</p>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-slate-800 bg-slate-900/50">
                ...
                <th className="text-left text-xs font-semibold text-slate-500 px-4 py-3">Sent At</th>
                <th className="text-right text-xs font-semibold text-slate-500 px-4 py-3">Cost</th>
              </tr>
            </thead>
            <tbody>
              {recentLogs.map(log => (
                <tr key={log.id} className="border-b border-slate-800/40 hover:bg-slate-800/20 transition-colors">
                  <td className="px-4 py-3 font-medium text-slate-200">{log.client}</td>
                  <td className="px-4 py-3 text-slate-400 font-mono text-xs">{log.to}</td>
                  <td className="px-4 py-3 text-slate-400">{log.template}</td>
                  <td className="px-4 py-3">
                    <span className={`text-xs font-semibold px-2 py-0.5 rounded-full border ${STATUS_STYLES[log.status]}`}>{log.status}</span>
                  </td>
                  <td className="px-4 py-3 text-slate-500 text-xs">{new Date(log.sentAt).toLocaleString("en-GB")}</td>
                  <td className="px-4 py-3 text-right text-slate-400 text-xs">{log.cost}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {tab === "settings" && (
        <div className="max-w-lg space-y-4">
          <div className="philix-card p-5 space-y-4">
            <h3 className="font-semibold text-slate-200 flex items-center gap-2"><Settings size={16} className="text-indigo-400" /> SMS Gateway Configuration</h3>
            <div>
              <label className="text-sm font-medium text-slate-400 mb-1.5 block">Gateway Provider</label>
            ...
            <div>
              <label className="text-sm font-medium text-slate-400 mb-1.5 block">Sender ID <span className="text-slate-600">(max 11 chars)</span></label>
          ...
          <div className="philix-card p-4 text-xs text-slate-500 space-y-1">
            <p className="font-semibold text-slate-400">Cost estimate at scale:</p>
            <p>• 100 clients × K0.10/SMS = K10.00 per campaign</p>
            <p>• Daily automated reminders: ~K5–K20/day at 50–200 active loans</p>
            <p>• Register at <span className="text-indigo-400">developers.mtn.com</span> or <span className="text-indigo-400">africastalking.com</span></p>
          </div>
        </div>
      )}
    </div>
